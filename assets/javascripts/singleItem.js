...
function mealItemList(mealData) {
    var mealItemContainer = document.getElementById('main-container');
    ...
    let mealItem = `<div id="main">
    <h1 id="meal-item-name">${mealData.name} Recipe</h1>
    ...
    <h4>RECIPE VIDEO</h4>
    <iframe id="meal-item-video" src="https://www.youtube.com/embed/${mealData.url}?autoplay=1" frameborder="0" allowfullscreen="allowfullscreen"></iframe>
    <br><br>
    ...
    <div id="recipe-card">
        ...
        <h5>INGREDIENTS:</h5>
        <ul>
           ${datas.ingredient.map((list)=>{ 
               return `<li>${list}</li>`
            },).join('')}
        </ul>
        <h5>RECIPE STEPS:</h5>
        <ul id="meal-item-recipe-step">
        ${datas.steps.map((list)=>{ 
            return `<li>${list}</li>`
         },).join('')}
        </ul>
    </div>
